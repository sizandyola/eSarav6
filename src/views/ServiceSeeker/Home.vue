...
export default  {
  name: 'Tab1',
  components: { ExploreContainer, IonHeader, IonToolbar, IonTitle, IonContent, IonPage, Carousel },
  data(){
    return{
      loading: true,
      services: []
...
methods:{
   getServices() {
    ...
    goTo(item){
      this.$router.push({name: "serviceDetails",params:{item:JSON.stringify(item)}})
    },
    notifications(){
